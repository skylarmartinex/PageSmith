...
export function ElegantTemplate({ content, config }: ElegantTemplateProps) {
    return (
        <div className="max-w-3xl mx-auto min-h-screen" style={{ backgroundColor: config.colors.background, fontFamily: config.fontFamily, color: config.colors.text }}>
            {/* Cover */}
            <div className="relative text-center px-16 py-20 pb-16 border-b overflow-hidden" style={{ borderColor: config.colors.accent + "40" }}>
                {content.coverImage && (
                    <>
                        <img src={content.coverImage.url} alt={content.coverImage.alt} className="absolute inset-0 w-full h-full object-cover" style={{ filter: "brightness(0.07) saturate(0.3)" }} />
                        <div className="absolute inset-0" style={{ backgroundColor: config.colors.background + "e0" }} />
                    </>
                )}
                <div className="relative">
                    ...
                    <h1 className="text-5xl font-bold leading-tight mb-6 italic" style={{ color: config.colors.primary }}>{content.title}</h1>
                    ...
                    {content.author && <p className="mt-4 text-sm italic" style={{ color: config.colors.secondary }}>by {content.author}</p>}
                </div>
            </div>
